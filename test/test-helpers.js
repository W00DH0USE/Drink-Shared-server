...
function cleanTables(db) {
  return db.transaction(trx =>
    ...
    .then(() =>
      Promise.all([
        trx.raw(`ALTER SEQUENCE sociare_articles_id_seq minvalue 0 START WITH 1`),
        trx.raw(`ALTER SEQUENCE sociare_users_id_seq minvalue 0 START WITH 1`),
        trx.raw(`ALTER SEQUENCE sociare_comments_id_seq minvalue 0 START WITH 1`),
        trx.raw(`SELECT setval('sociare_articles_id_seq', 0)`),
        trx.raw(`SELECT setval('sociare_users_id_seq', 0)`),
        trx.raw(`SELECT setval('sociare_comments_id_seq', 0)`),
      ])
    )
  )
}
...
function seedUsers(db, users) {
  ...
  return db.into('sociare_users').insert(preppedUsers)
    .then(() =>
      // update the auto sequence to stay in sync
      db.raw(
        `SELECT setval('sociare_users_id_seq', ?)`,
...
function seedArticlesTables(db, users, articles, comments=[]) {
  ...
  return db.transaction(async trx => {
    ...
    await trx.into('sociare_articles').insert(articles)
    // update the auto sequence to match the forced id values
    await trx.raw(
      `SELECT setval('sociare_articles_id_seq', ?)`,
      [articles[articles.length - 1].id],
    )
    // only insert comments if there are some, also update the sequence counter
    if (comments.length) {
      await trx.into('sociare_comments').insert(comments)
      await trx.raw(
        `SELECT setval('sociare_comments_id_seq', ?)`,
